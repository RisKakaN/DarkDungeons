...
Tile::Tile(SDL_Texture *tileset, Vector2 size, Vector2 tilesetPosition, Vector2 position) :
        ...
        position(Vector2(position.x * game_constants::TILE_SCALE, position.y * game_constants::TILE_SCALE)) {}

void Tile::update(float elapsedTime) {}

void Tile::draw(Graphics &graphics) {
    SDL_Rect desRect = {this->position.x, this->position.y, this->size.x * game_constants::TILE_SCALE,
                        this->size.y * game_constants::TILE_SCALE};
